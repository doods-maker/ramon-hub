...
| `enterprise/app/services/messages/audio_transcription_service.rb` | **RAIZ do conflito acima:** a transcrição deixa de ler as `CAPTAIN_OPEN_AI_*` — `uri_base` e `setup_model` sobrescritos com destino próprio (`WHISPER_ENDPOINT`/`WHISPER_MODEL`, envs opcionais `RAMON_WHISPER_ENDPOINT`/`RAMON_WHISPER_MODEL`). Com isso as `CAPTAIN_OPEN_AI_*` voltam a ser do chat do Captain e param de mandar auto-resolver/copiloto/tradução/tagline/Agents SDK pro container de transcrição. A `CAPTAIN_OPEN_AI_API_KEY` segue compartilhada (o base monta o client no `initialize`; o speaches ignora o token) | whisper-config 25/07 |
...
| `app/controllers/api/v1/accounts/leads_controller.rb` + `config/routes.rb` | `for_conversation` vira readonly em caixa com Portaria (abrir a conversa não cria lead); nova collection route `encaminhar_comercial` (time `comercial` + `find_or_create_lead_for` + canal `indicacao`) | Encaminhar ao comercial | portaria 28/08 |
| `api/leads.js` + `store/modules/leads.js` + `ramon/components/conversation/LeadConversationPanel.vue` + `ramon.json` en/pt_BR | `ensureForConversation` trata 204 (não mescla vazio); action `encaminharComercial`; estado "sem lead" no painel com botão só quando `getSelectedChat.meta.team.name === 'recepção'` | Encaminhar ao comercial (UI) | portaria 28/08 |
...
| `enterprise/lib/captain/tools/advbox_mcp_escrita_tool.rb` + filhas `criar_{tarefa,movimentacao,cliente}_advbox_tool.rb` + `configuracoes_advbox_tool.rb` (+ spec) | escrita no AdvBox pelo Copiloto em 2 tempos: sem `codigo` = prévia + hash dos dados; com o código (após ok do humano) = grava via MCP; só tarefa/movimentação/cliente (`PERMITIDAS`); base ganhou `escrita?` + coerção de boolean | Trilha 1b (escrita com confirmação) | copiloto 18/08 |
| `enterprise/lib/captain/tools/advbox_mcp_tool.rb` + 9 filhas `{processo,movimentacoes,historico_tarefas,ultimas_movimentacoes,tarefas,buscar_cliente,cliente,documentos,link_documento}_advbox_tool.rb` (+ `spec/enterprise/lib/captain/tools/advbox_mcp_tool_spec.rb`) | ponte Copiloto→MCP do AdvBox: `mcp_tool 'advbox_x'` herda descrição/params/execução de `Ramon::AdvboxMcpService` (só LEITURA; escrita recusada na classe); `buscar_processo_advbox` ganhou `numero_processo`; +9 ids em `tools.yml`; skill "Consultas no AdvBox" no seed do Copiloto | Trilha 1 (mais tools ADVBOX no Copiloto) | copiloto 18/08 |
