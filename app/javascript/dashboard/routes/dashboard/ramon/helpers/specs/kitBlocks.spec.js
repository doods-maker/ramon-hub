...
describe('stageMode', () => {
  it('é encerrado quando o lead tem won_at ou lost_at', () => {
    expect(stageMode({ won_at: '2026-07-04', stage_name: 'Fechado' })).toBe(
      'encerrado'
    );
    expect(stageMode({ lost_at: '2026-07-04', stage_name: 'Perdido' })).toBe(
      'encerrado'
    );
  });
  ...
  it('é sdr por padrão (Novo, Qualificação, desconhecida, sem etapa)', () => {
...
describe('kitBlocks', () => {
  ...
  it('closer vê resumo, venda/objeções, documentos e próximo passo', () => {
    expect(kitBlocks('closer')).toEqual([
      'resumo',
      'venda_objecoes',
      'documentos',
      'proximo_passo',
