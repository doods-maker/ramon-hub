...
escreve uma nota privada começando com `@claude`. O runner roda
`claude -p --bare` (só leitura: Read/Grep/Glob + tools `mcp__advbox__*` de
consulta) e faz as escritas determinísticas: arquivo no Drive → tarefa ADVBOX
